import { observer } from 'mobx-react-lite';
import { MdChevronLeft } from 'react-icons/md';
import { useHistory } from 'react-router-dom';
import { useStore } from 'store';

export default observer(() => {
  const history = useHistory();
  const { pathStore } = useStore();

  return (
    <div>
      <div className="h-[44px] w-screen" />
      <div className="h-[44px] bg-white dark:bg-[#181818] flex items-center border-b dark:border-white dark:md:border-opacity-10 dark:border-opacity-[0.05] border-gray-ec fixed top-0 left-0 right-0 z-[20]" onClick={() => {
        pathStore.prevPath ? history.goBack() : history.push(`/`)
      }}>
        <div className="flex items-center text-30 ml-2 opacity-70">
          <MdChevronLeft />
        </div>
        <span className="mt-[2px] ml-[1px] text-15 opacity-70 leading-none">返回</span>
      </div>
    </div>
  )
})