...
const Main = observer((props: IModalProps) => {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        await sleep(300);
        const { url } = await TweetApi.get(groupStore.map.group_timeline.groupId, userStore.address);
        state.url = url || '';
  ...
  const submit = async () => {
    ...
    try {
      if (state.isUpdating) {
        if (state.url === '') {
          confirmDialogStore.show({
            ...
            ok: async () => {
              ...
              await sleep(400);
              await TweetApi.remove(groupStore.map.group_timeline.groupId, userStore.address);
              await sleep(1000);
          ...
          state.submitting = true;
          await sleep(300);
          await TweetApi.update(groupStore.map.group_timeline.groupId, userStore.address, {
            url: state.url
          });
          await sleep(1000);
          state.submitting = false;
        ...
        state.submitting = true;
        await sleep(300);
        await TweetApi.create(groupStore.map.group_timeline.groupId, {
          url: state.url,
